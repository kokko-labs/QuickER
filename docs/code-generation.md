...
- **Two columns.** A `!=` between two columns becomes `(a <> b OR (a IS NULL AND b IS NOT NULL) OR (a IS NOT NULL AND b IS NULL))`, matching C# (and EF Core) in counting one `NULL` side as different and two `NULL` sides as the same. A column-to-column `==` stays a plain `=`, where SQL and C# already agree that a `NULL` side is not a match.
- **Negation.** `!(a == b)` and `!(a != b)` flip the operator instead of being wrapped in `NOT (...)`, so they get exactly the compensation the opposite operator gets on its own. Negating anything else still produces `NOT (...)`.
