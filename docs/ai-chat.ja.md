...
- **添付**：ファイルを添付して、既存の設計資料などを文脈として渡せます。
  種別ごとにサイズ上限があり、添付できる種別は接続方式によって変わります。
  - **OpenAI 互換**：画像・テキスト
  - **Anthropic API**：画像・PDF・テキスト
  - **Claude Code**：画像・PDF・テキストと、その他のバイナリ
  - **Copilot**：画像のみ
  - **Codex**：添付に非対応
...
「AI モック生成」は、現在の ER 図から Web モック画面を起こします。
...
モックフォルダからは、画面設計書（`README.md`＝画面一覧・Mermaid の遷移図・画面×エンティティの CRUD 表・画面ごとの項目表）を AI を使わず決定的に生成することもできます。
...
生成はソリューション・csproj・`README-QuickER.md`・`Generated/`・`design/mock/` を同名で上書きし（同じプロジェクト名での作り直しはこれで行えます）、それ以外のファイルはそのまま残します。
...
- **最終ビルドは、エージェントのサンドボックスの外側で、あなたの権限で実行されます＝境界を越える唯一の地点です。**
