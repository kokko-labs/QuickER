...
任意の第 2 ステップ（Claude Code 限定）として、モックフォルダから実行可能な **WPF モックプロジェクト**を生成できます。モックフォルダは `design/mock/` に同梱され、Claude Code がそれを基に WPF の UI を実装します。追加指示欄から、その実装への追加の指示を渡せます。
