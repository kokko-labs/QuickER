...
- **Command timeout** — how long a single schema-import or sync statement may run. It defaults to 60 seconds and applies to every dialect including SQLite; `0` means no limit (the ADO.NET convention), and negative values are rejected when you confirm the dialog. It is saved per connection profile, and profiles written before this setting existed load with the default
