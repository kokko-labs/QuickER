...
結果は新規の図として書き出し、既存図へのマージは行いません。`--out` が新しいフォーマットで保存された図を指す場合は、何も書かずに終了コード `2` で止まります（このバージョンの形式で書き出すと、このバージョンが表現できないデータが失われるため）。致命的でない指摘は警告として標準エラーへ出力し、解析対象クラスが 0 件の場合は失敗します。
...
`EcOrder.g.md`（英語版）も同梱出力されます（チェックイン済み・ドリフト検知の対象）。代わりに日本語版
`{ベース名}.ja.g.md` を出すには `--api-docs-lang Japanese`、両方なら `--api-docs-lang Both` を追加します（どちらも `--generate-api-docs` が前提）。
