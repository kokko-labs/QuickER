...
| `get_diagram_summary` | — | Return a text listing of the tables, columns, unique constraints, and relationships in the diagram. Each relationship line shows its column pairs and constraint name — `Customer → Order (OneToMany, FK: (CustomerId → CustomerId)) [FK_Order_Customer]` — so a composite foreign key is listed with every pair in declaration order |
