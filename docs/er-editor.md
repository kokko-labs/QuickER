...
Express a composite primary key by checking PK on multiple columns — which columns make up the key is always the PK checkbox's business. The key's own column order is edited separately, in a "Primary key order" card the properties panel places between the column grid and the unique constraints. The card appears only while the selected table has two or more primary key columns, and holds one row per column in key order, with ↑ / ↓ to move a row (the buttons are disabled at the ends). That order is the order of the generated `PRIMARY KEY` constraint and of the index behind it, and one move is one undo step. Until the order has been stated explicitly it simply follows the column order in the grid, so a diagram built in the editor behaves as it always has: drag rows and the key follows. Stating it once pins it — by moving a row in the card, by the `set_primary_key` AI tool, or by importing the table from a database — and from then on two things hold: reordering columns in the grid no longer changes the key's order, and a column whose PK checkbox you tick afterwards joins the key at the end (a column that was part of the pinned order before returns to its old place instead).
...
- **Full screen** — "Full screen" (F11) hides the window frame and fills the screen. The toolbar and status bar stay, so combining it with the panel toggles below leaves you with just the canvas. Exit with F11 or the same toggle in the "View" group; this is the one state that is not restored on the next launch. Since the title bar is hidden, an "Exit" button appears at the right end of the toolbar in its place (it closes the app exactly as the title bar's close button does: no confirmation, auto-save first)
