...
| `add_column` | `table_name` ✅, `column_name` ✅, `data_type` ✅, `is_primary_key`, `is_nullable`, `description` | テーブルへカラムを追加する。主キー列は各テーブルにちょうど 1 つにする。ツールは 2 本目を拒否しないが、コード生成器が複合主キーに対応していない。`is_primary_key=true` で追加した列は `is_nullable` の指定に依らず常に NOT NULL になる。複合主キーの宣言や、既存テーブルの主キーの変更には `set_primary_key` を使う |
...
| `set_column_property` | `table_name` ✅, `column_name` ✅, `description`, `data_type`, `is_nullable` | カラムの説明・データ型・NULL 許容を変更する（いずれか 1 つ以上を指定）。主キー列は `is_nullable` の指定に依らず NOT NULL のままになる（`add_column`・`set_primary_key` と同じ正規化。どうしても NULL 許容にしたいときは先に主キーから外す） |
...
| `set_primary_key` | `table_name` ✅, `columns` ✅（列名の配列・主キーの列順） | テーブルの主キーを設定する。照合キーは `table_name` のみ＝主キーはテーブルに高々 1 つのため、呼び出しは常に現在の主キーを置き換える（挙げた列が指定順で主キーになり、それ以外の列は主キーから外れる）。`set_unique_constraint` と違って列の順序に意味があり、生成される `PRIMARY KEY` 制約の列順そのものになる（テーブルの列順と食い違うときは `get_diagram_summary` が注記する）。挙げた列は NOT NULL になる。主キーから外れた列の NULL 許容は据え置かれる。リレーションとの整合は検査しないため、リレーションが参照する列を主キーから外すと、その外部キーはどのキーにも一意制約にも覆われない列を参照する形になり、生成した DDL の適用時に失敗する（そうした列は主キーに残すか、`set_unique_constraint` で覆う）。列は最低 1 つ必要なため、このツールでテーブルの主キーを無くすことはできない |
