...
- **C# code generation** — in addition to Entity / EditModel / Mapper, choose a data-access layer to generate:
  ...
  - **Remote-capable interfaces (--remote-contracts)** — an option that additionally generates `I{Entity}RemoteRepository` with only the operations that could be served over a web service in the future (CRUD, save, and named queries). `I{Entity}Repository` keeps all methods and inherits it, so existing code is unaffected; keep your application code dependent only on the remote surface and a future switch to a remote implementation stays compile-time safe
