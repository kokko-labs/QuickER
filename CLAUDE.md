...
- **store-generated 列の書き込み除外（rowversion / timestamp）**: DB が値を生成する列は生成 Entity にマーカー属性 `[StoreGeneratedColumn]` を刻み（判定は型マッパーの `CSharpTypeInfo.IsRowVersion`＝`rowversion`/`timestamp`・付与はオプション非依存で無条件）、`EntitySaveMetadata` が Repository (QuickER) の INSERT / BulkInsert / UPDATE の対象から除外する（`InsertProperties`＝全列−store-generated・`NonKeyProperties` からも除外）。DB が採番するため明示挿入は実行時エラー（`Cannot insert an explicit value into a timestamp column.`）になるのを防ぐ。**SELECT では取得する**（`SelectProperties`/`AllProperties` は不変＝並行性トークンは読める）。EF Core は Fluent の `IsRowVersion()` が既に store-generated として扱うため非適用・InMemory は SQL 非使用で無関係。楽観排他（UPDATE の WHERE に rowversion を使う）はスコープ外。属性定義は `[UnboundedBinaryColumn]` と同型の emit ゲートで常時出力（固定 infra が参照するため）
- **EF Core モード（GenerateEfCore）**: 既存 Entity をそのまま EF に乗せる方言非依存の QuickErDbContext＋EF 版 Repository を生成し、DI 登録（AddGeneratedRepositories ⇔ AddGeneratedEfCoreRepositories）の差し替えだけでQuickER の SQL Server 実装と交換できる。GenerateEfCore=false のとき生成物に EF への依存は一切出ない（using 含む）。スキーマ作成は従来どおり DDL 生成の責務で、EF は既存スキーマへの接続専用（Migrations / HasColumnType は範囲外）
...
- **無制限バイナリ列の除外（ExcludeUnboundedBinaryColumns）**: `varbinary(max)` / `image` / 長さ宣言なし BLOB / `bytea` 等の**無制限バイナリ列**を、Repository (QuickER) の SELECT / UPDATE 対象から外す bool オプション（既定 false。CLI `--exclude-unbounded-binary`／GUI チェック）。判定は**宣言ベース**でプロバイダの型マッパー（`CSharpTypeInfo.IsUnboundedBinary`＝`varbinary(max)`/`image`/長さなし BLOB は真・`rowversion`/`timestamp`/`binary(n)`/`varbinary(n)` は偽）が行い、生成 Entity のプロパティへマーカー属性 `[UnboundedBinaryColumn]` を刻む（除外列一覧は Info 診断で通知）。ランタイム（`EntitySaveMetadata`）は属性をリフレクションで読み列集合を分割する＝**INSERT / BulkInsert は全列**（初回書き込みで blob を渡せる）・**SELECT は縮小列**（除外列は未取得状態＝nullable は null・非 nullable は空配列）・**UPDATE は縮小列**（除外列に触れず既存 blob を温存）。除外列に「null でも空でもない」値を残したまま UPDATE / SaveAsync（グラフカスケード）/ InMemory 更新 / リモートクライアント送信を試みると `InvalidOperationException`（サイレントなデータ取りこぼしを防ぐ・除外列の更新は生 SQL `ExecuteSqlAsync` へ誘導）。生 SQL のエンティティ取得は縮小列が必須だが、SELECT に除外列を含めれば opportunistic にマップされる。名前付きクエリの DSL 射影が除外列を参照していればサーバー側刈り込み SELECT に含まれ値が取れる（WHERE 参照も可）。**EF Core（DbSet 経由）は非適用**＝EF の列選択は EF の責務のため全列読み書き。InMemory は完全パリティ（読み取り複製の除外列 strip・更新ガード・射影は完全クローンから）。除外が効く図でも呼び出しごとに blob 込みで取得できる**読み取りオプトイン `SqlQuery<T>.WithUnboundedBinary()`**（`SqlQueryPlan` の bool フラグ・API は常時生成で除外列なしは no-op）を Query() に持ち、ON かつ Include なしのとき SQL Server は FOR JSON を使わず全列プレーン SELECT（Base64 膨張回避）・SQLite も全列 SELECT・InMemory は strip なしの完全クローンを返し（RowState=Unchanged の正当なエンティティ）、EF Core は元々全列のため no-op。Include との併用は終端で `InvalidOperationException`（両順序を BuildPlan で検出）。効果は ToListAsync / FirstOrDefaultAsync のみ（取得後の除外列更新は既存 UPDATE ガードで throw＝生 SQL へ誘導）。VO・マルチターゲット・EF Core・InMemory・リモートと直交。除外列ごとに**ストリーミング Stream アクセサ `Read/Write{Column}Async`**（＋ファイル糖衣 `Read/Write{Column}ToFile/FromFileAsync`）も追加生成する（`GenerateRepositories && ExcludeUnboundedBinaryColumns` かつ除外列があるときのみ・全機能面 `I{Entity}Repository` 限定でリモート面には載せない）＝O(チャンク)・blob 全量をメモリに載せない唯一の手段。契約・薄い委譲実装（QuickER 2 方言＋InMemory は固定 infra のエンジンへ委譲・列名は `EntitySaveMetadata` 経由で焼き込まない）・EF は `NotSupportedException`・ファイル糖衣は拡張メソッド静的クラス（Stream 版へ 1 行委譲）を `CSharpGenerationModelBuilder.BinaryStream.cs` が整形済みブロックとして組み立てる。エンジンは SQL Server=`SequentialAccess`＋`GetStream`／Stream 値の `SqlParameter`（`VarBinary`,-1）・SQLite=`SqliteBlob`＋`zeroblob`（rowid 前提・QuickER の SQLite DDL は WITHOUT ROWID 非生成）。`CanSeek` でない Stream は `length` 必須（方言中立に統一・欠落は `ArgumentException`）。`source=null` で SET NULL・行なし/NULL は false。INSERT 専用メソッドはなく「INSERT→Write の 2 段」。楽観排他はスコープ外。実 DB 全経路検証は BinaryColumnAdo/EfCore/Remote/InMemory 各テスト（WithUnboundedBinary の取得・Include 併用例外・取得後 Update ガード・Stream の往復/SET NULL/行なし/非シーク length 必須/ファイル糖衣往復を含む・CI 常時実行）
- **DB アクセスの排他と単独出力**: Repository バケットは「共通契約」（GenerateRepositories || GenerateEfCore で出力）と「Repository (QuickER) 実装」（GenerateRepositories 時のみ）に分割されており、**GenerateEfCore 単独の生成物にはQuickER 実装の ADO 依存（Microsoft.Data.SqlClient / Microsoft.Data.Sqlite）が一切出ない**（ガードテストあり）。Repository (QuickER) は `RepositoryDialect` で方言を選ぶ多DB対応（既定 `sqlserver`＝FOR JSON／`sqlite`＝プレーン SELECT＋IncludeLoader マルチクエリ・LIMIT/OFFSET・strftime）で、生成時に片方言のみを出力し方言間の ADO 依存も排他（ガードテスト双方向）。`RepositoryDialects`（リスト・単一 `RepositoryDialect` の後方互換上位）に複数方言を指定するとマルチターゲット出力になり、中立契約（IRepository / I{Entity}Repository / ISqlExecutor など）を `RepositoryNamespace` に 1 回・方言別実装を `.SqlServer` / `.Sqlite` の方言別 namespace に出力する。DI は方言別拡張＋keyed 版（AddGeneratedSqlServerRepositories / AddGeneratedSqliteRepositories・`object? serviceKey` 付き）で、単一契約型を keyed 解決して同一プロセスで複数 DB へ書き分けられる（実 DB 結合は MultiTargetRepositoryRuntimeTests＝Docker 依存・keyed 解決の型検証は MultiTargetRepositorySqliteKeyedResolutionTests＝CI 実行）。マルチターゲット（実効方言 2 つ以上）と GenerateEfCore は排他で、併用指定は診断エラーになる。GUI の生成ダイアログは DB アクセスをラジオ3択（なし既定／Repository (QuickER)＝常時選択可・対象 DB チェック群 [SQL Server / SQLite] で選択（既定=図の方言、未対応方言の図では空・最低1つ必須）／EF Core）で排他選択させる。両方 ON はパリティ検証用に CLI／オプション直指定でのみ可能
