...
PolyForm Noncommercial 1.0.0（パッケージ同梱の LICENSE-NC.md）。**現在は商用利用を含め全員が無料**で利用できます。将来のバージョンでは、一部機能を有償ライセンス化する可能性があります（基本生成＝Entity / EditModel / Mapper は商用含め永続無料／既存機能の個人・非商用利用は無料のまま／公開済みバージョンに付与した権利は遡って取り消しません／有償化する場合は事前に告知し、既存利用者には移行期間を設けます）。詳細はリポジトリの LICENSING.ja.md を参照してください。
