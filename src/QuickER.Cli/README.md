...
The command-line tool for [QuickER](https://github.com/kokko-labs/QuickER), an ER diagram designer for .NET. It generates C# code from an ER diagram, scaffolds code straight from a live database, turns generated C# back into a diagram, and hosts a stdio Model Context Protocol (MCP) server that gives AI agents the same operations. Requires .NET 10.
...
Target databases are SQL Server, PostgreSQL, MySQL, Oracle, and SQLite.
...
Every generation option has a kebab-case flag of the same name, and flags override the settings file passed with `--config`. The CLI reference lists them all.
...
The license is PolyForm Noncommercial 1.0.0 plus additional grants, bundled with the package as `LICENSE-NC.md`. The grants are what open up commercial use, and rights granted for a released version are never withdrawn retroactively. [LICENSING.md](https://github.com/kokko-labs/QuickER/blob/main/LICENSING.md) explains the arrangement in plain language.
