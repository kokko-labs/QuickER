...
public sealed class OracleDdlGenerator : IDdlGenerator
{
    ...
    /// <returns>Oracle 用の DDL スクリプト</returns>
    public string Build(ErDiagram diagram)
    {
        ...
        foreach (var entity in diagram.Entities)
        {
            ...
            sb.AppendLine($"CREATE TABLE {OracleIdentifier.Quote(table)} (");
            ...
            for (var i = 0; i < entity.Columns.Count; i++)
            {
                ...
                var line =
                    $"    {OracleIdentifier.QuoteSimple(col.Name)} {col.DataType} {(col.IsPrimaryKey || !col.IsNullable ? "NOT NULL" : "NULL")}";
            ...
            if (pks.Count > 0)
            {
                var pkCols = string.Join(
                    ", ",
                    pks.Select(p => OracleIdentifier.QuoteSimple(p.Name))
                );
                sb.AppendLine(
                    $"    CONSTRAINT \"PK_{OracleIdentifier.SafeName(table)}\" PRIMARY KEY ({pkCols})"
                );
            }
        ...
        foreach (var rel in diagram.Relationships)
        {
            ...
            {
                fkColName = OracleIdentifier.SafeName(pkEntity.TableName) + "_" + pkCol.Name;
            }
            ...
            var constraintName = string.IsNullOrWhiteSpace(rel.ConstraintName)
                ? $"FK_{OracleIdentifier.SafeName(fkTable)}_{OracleIdentifier.SafeName(pkTable)}"
                : rel.ConstraintName;

            // ON UPDATE が指定されていても Oracle では無視される旨を注意コメントで残す
            if (rel.OnUpdate != ForeignKeyReferentialAction.NoAction)
            {
                sb.AppendLine("-- 注: Oracle は ON UPDATE をサポートしないため無視");
            }

            var deleteClause = OracleReferentialAction.BuildOnDeleteClause(rel.OnDelete);

            sb.AppendLine(
                $"ALTER TABLE {OracleIdentifier.Quote(fkTable)} ADD CONSTRAINT \"{OracleIdentifier.Escape(constraintName)}\" "
                    + $"FOREIGN KEY ({OracleIdentifier.QuoteSimple(fkColName)}) REFERENCES {OracleIdentifier.Quote(pkTable)} ({OracleIdentifier.QuoteSimple(pkCol.Name)}){deleteClause};"
            );
        }
