...
public sealed partial class SqliteDdlGenerator : DdlGeneratorBase
{
    ...
    public override string Build(ErDiagram diagram)
    {
        ...
        // （基底の Build を経由しないインライン制約方式なので、ここで同じ関門を通す。
        //  後段の FormatDeclaredType は「SQLite の型名文法に載せるためのクォート」で検証ではない）
        ...
        foreach (var rel in diagram.Relationships)
        {
            ...
            {
                sb.AppendLine(
                    // テーブル名は自由入力なので改行・制御文字を畳んでコメント行の突き破りを防ぐ（基底と同一規則）
    ...
    )
    {
        ...
        {
            // テーブル名・説明とも自由入力なので改行・制御文字を畳んでコメント行の突き破りを防ぐ
            sb.AppendLine(
                $"-- {SqlComment.Sanitize(entity.TableName)}: {SqlComment.Sanitize(entity.Description)}"
            );
        }
        ...
        for (var i = 0; i < entity.Columns.Count; i++)
        {
            ...
            {
                // 列名・説明とも自由入力なので改行・制御文字を畳んでコメント行の突き破りを防ぐ
                sb.AppendLine(
                    $"    -- {SqlComment.Sanitize(col.Name)}: {SqlComment.Sanitize(col.Description)}"
                );
            }
        }
