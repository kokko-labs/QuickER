...
public sealed class MySqlDdlGenerator : IDdlGenerator
{
    ...
    /// <returns>MySQL 用の DDL スクリプト</returns>
    public string Build(ErDiagram diagram)
    {
        ...
        foreach (var entity in diagram.Entities)
        {
            ...
            sb.AppendLine($"CREATE TABLE {MySqlIdentifier.Quote(table)} (");
            ...
            for (var i = 0; i < entity.Columns.Count; i++)
            {
                ...
                var line =
                    $"    {MySqlIdentifier.QuoteSimple(col.Name)} {col.DataType} {(col.IsPrimaryKey || !col.IsNullable ? "NOT NULL" : "NULL")}";
            ...
            if (pks.Count > 0)
            {
                var pkCols = string.Join(
                    ", ",
                    pks.Select(p => MySqlIdentifier.QuoteSimple(p.Name))
                );
                sb.AppendLine(
                    $"    CONSTRAINT `PK_{MySqlIdentifier.SafeName(table)}` PRIMARY KEY ({pkCols})"
                );
            }
        ...
        foreach (var rel in diagram.Relationships)
        {
            ...
            {
                fkColName = MySqlIdentifier.SafeName(pkEntity.TableName) + "_" + pkCol.Name;
            }
            ...
            var constraintName = string.IsNullOrWhiteSpace(rel.ConstraintName)
                ? $"FK_{MySqlIdentifier.SafeName(fkTable)}_{MySqlIdentifier.SafeName(pkTable)}"
                : rel.ConstraintName;
            var referentialActions = BuildReferentialActionClause(rel);

            sb.AppendLine(
                $"ALTER TABLE {MySqlIdentifier.Quote(fkTable)} ADD CONSTRAINT `{MySqlIdentifier.Escape(constraintName)}` "
                    + $"FOREIGN KEY ({MySqlIdentifier.QuoteSimple(fkColName)}) REFERENCES {MySqlIdentifier.Quote(pkTable)} ({MySqlIdentifier.QuoteSimple(pkCol.Name)}){referentialActions};"
            );
        }
