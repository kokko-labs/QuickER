...
/// <remarks>
/// 外部キー列の名前解決（<see cref="ForeignKeyColumnResolver"/>）で用いる。GUI 側
/// <c>QuickER.Services.IdentifierNameHelper</c> の外部キー解決に必要な部分の移植で、
/// ファイルベースの MCP ホストが VM に依存せず同じ命名規則を再現するために持つ。
/// </remarks>
