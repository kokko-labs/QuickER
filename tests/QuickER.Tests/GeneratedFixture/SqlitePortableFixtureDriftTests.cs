...
/// <remarks>
/// <para>
/// 入力の図は方言可搬フィクスチャと同一だが、オプションが SQLite 方言のQuickER 版 Repository＋EF Core 併存
    ...
    public void CommittedSqliteFixture_MatchesRegeneratedOutput()
    {
        FixtureDriftHarness.VerifyOrRegenerate(
            ...
            "コミット済み SQLite フィクスチャが現在のテンプレート出力と乖離しています。"
                + "SqlitePortableFixtureDefinition（SQLite 方言・QuickER 版 Repository＋EF Core）から再生成が必要です。"
        );
    }
}
