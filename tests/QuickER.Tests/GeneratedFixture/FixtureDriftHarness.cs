...
/// </para>
/// </remarks>
internal static partial class FixtureDriftHarness
{
    ...
    )
    {
        ...
        if (IsRegenerationRequested())
        {
            ...
            WriteIfSubstantivelyChanged(checkedInPath, rendered);
        ...
        NormalizeGeneratorVersion(committed)
            .Should()
            .Be(
                NormalizeGeneratorVersion(rendered),
                $"{driftReason} 再生成先: {checkedInPath}。{RegenCommandHint}"
            );
    }

    /// <summary>
    /// 生成元の版の行（<c>// Generated by QuickER x.y.z</c> / <c>&lt;!-- Generated by QuickER x.y.z --&gt;</c>）の
    /// 版の部分を固定文言へ置き換える。
    /// </summary>
    /// <remarks>
    /// 版は <c>VersionPrefix</c> を上げるたびに変わり、ビルド環境によってはプレリリース接尾辞も付くため、
    /// ドリフト比較からは版の値だけを外す（行の有無・位置・書式は引き続き比較する）。内容ハッシュは版の行を
    /// 対象外として計算するため、版だけが違うフィクスチャのハッシュ行は一致する。
    /// </remarks>
    private static string NormalizeGeneratorVersion(string content) =>
        GeneratorVersionLine().Replace(content, "${prefix}{version}${suffix}");

    [GeneratedRegex(
        @"^(?<prefix>(?://|<!--) Generated by QuickER )\S+(?<suffix> -->)?(?=\r?$)",
        RegexOptions.Multiline
    )]
    private static partial Regex GeneratorVersionLine();

    /// <summary>
    /// 再生成モードで書き出す。版の行以外に差分が無ければ書き込まない
    /// （版を上げただけでチェックイン済みファイルが一斉に書き換わるのを避ける）。
    /// </summary>
    private static void WriteIfSubstantivelyChanged(string path, string content)
    {
        if (
            File.Exists(path)
            && NormalizeGeneratorVersion(File.ReadAllText(path))
                == NormalizeGeneratorVersion(content)
        )
        {
            return;
        }
    ...
    )
    {
        ...
        if (IsRegenerationRequested())
        {
            ...
            WriteIfSubstantivelyChanged(fixturePath, regenerated);
        ...
        NormalizeGeneratorVersion(committed)
            .Should()
            .Be(
                NormalizeGeneratorVersion(regenerated),
                $"{driftReason} 再生成先: {fixturePath}。{RegenCommandHint}"
            );
    }
