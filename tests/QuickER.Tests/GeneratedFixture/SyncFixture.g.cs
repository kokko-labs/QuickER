...
// Content hash: sha256:39283d24e10616e7d8db525888d62df75105146778a656d4b2593df3a92f13cc
#nullable enable
...
{
    /// <summary>Projects order headlines (id and customer name) above the given id</summary>
    Task<IReadOnlyList<SyncOrderHeadlineRow>> GetHeadlinesAsync(int minOrderId, CancellationToken cancellationToken = default);
...
/// <summary>Projection DTO for the named query GetHeadlines (sync_orders).</summary>
public sealed partial class SyncOrderHeadlineRow
{
...
{
    /// <summary>Projects order headlines (id and customer name) above the given id</summary>
    public Task<IReadOnlyList<SyncOrderHeadlineRow>> GetHeadlinesAsync(int minOrderId, CancellationToken cancellationToken = default) =>
        InvokeAsync<IReadOnlyList<SyncOrderHeadlineRow>>("GetHeadlines", new { minOrderId }, cancellationToken);
    ...
    : JournalingRepository<SyncOrderEntity, int>, ISyncOrderRepository
{
    ...
    /// <inheritdoc />
    public Task<IReadOnlyList<SyncOrderHeadlineRow>> GetHeadlinesAsync(int minOrderId, CancellationToken cancellationToken = default) =>
        _inner.GetHeadlinesAsync(minOrderId, cancellationToken);
}
...
internal static class UnboundedBinaryColumnEngine
{
    ...
    )
    {
        ...
        await using var command = CreateCommand(
            $"SELECT {quotedColumn} FROM {metadata.TableName} WHERE {EntitySaveMetadata.QuoteIdentifier(metadata.KeyColumnName)} = @id;",
            connection,
            transaction
        );
    ...
    )
    {
        ...
        await using var command = CreateCommand(
            $"UPDATE {metadata.TableName} SET {quotedColumn} = @data WHERE {EntitySaveMetadata.QuoteIdentifier(metadata.KeyColumnName)} = @id;",
            connection,
            transaction
        );
...
{
    /// <summary>The projection selector of GetHeadlinesAsync, hoisted into a field so that every call hands the same expression tree instance to the executor (which compiles it once).</summary>
    private static readonly Expression<Func<SyncOrderEntity, SyncOrderHeadlineRow>> _getHeadlinesAsyncSelector = e => new SyncOrderHeadlineRow { OrderId = e.OrderId, CustomerName = e.CustomerName };

    /// <summary>Projects order headlines (id and customer name) above the given id</summary>
    public Task<IReadOnlyList<SyncOrderHeadlineRow>> GetHeadlinesAsync(int minOrderId, CancellationToken cancellationToken = default) =>
        Query().Where(e => e.OrderId > minOrderId).ToProjectionListAsync(_getHeadlinesAsyncSelector, cancellationToken);
