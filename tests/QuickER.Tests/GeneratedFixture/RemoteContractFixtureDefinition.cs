...
/// <c>GenerateRemoteContracts = true</c> を加えたもの。SQLite 方言の自作 Repository＋EF Core 併存のため、
/// リモート面（I{Entity}RemoteRepository）・全機能面（I{Entity}Repository）・両面 DI 登録を
/// 自作・EF の両実装で実ファイル DB（Docker 不要＝CI 常時実行）検証できる。
/// </para>
/// <para>
...
public static class RemoteContractFixtureDefinition
{
    ...
    /// <summary>フィクスチャ生成に用いる決定的なオプション（SQLite 方言・自作＋EF 併存・VO 有効・リモート契約生成）</summary>
