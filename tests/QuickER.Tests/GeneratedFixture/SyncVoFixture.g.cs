...
// Content hash: sha256:a186c19385bc0066a9c645e058ba6f87ca14d173c5f6c69dbf87c004bd2298ae
#nullable enable
    ...
    where TSelf : IValueObject<TSelf>
{
    ...
    /// claims absent inputs through <see cref="TryConvertAbsentInput"/>, in which case the result is that instance. A value
    ...
    /// <summary>Converts a value from outside the model and creates the value object from it (throws ValueObjectValidationException on violation; an absent value returns null unless claimed through <see cref="TryConvertAbsentInput"/>).</summary>
    /// <param name="raw">The value read from the source.</param>
    /// <param name="provider">The culture the text is written in; <c>null</c> uses the invariant culture.</param>
    ...
    /// input; return false and the call keeps its ordinary result, success with a null value object.
    /// </para>
    /// <para>
...
{
    /// <summary>One declared instance: the raw value, the instance itself, and the [Display] name when present.</summary>
    ...
    where TSelf : ValueObjectBaseCore<TSelf, TValue>, IValueObject<TSelf, TValue>
{
    ...
    /// (<see cref="IValueObject{TSelf}.TryConvertAbsentInput"/>) before falling back to the null result. The conversion to
    ...
    )
    {
        ...
        // writes one of its values as a blank claims it through TryConvertAbsentInput; the consult goes through TSelf so it
        // is honored no matter how this method is called (the same reason TryConvertCustomInput below goes through TSelf).
        if (raw is null or DBNull || (raw is string empty && empty.Length == 0))
        {
            result = TSelf.TryConvertAbsentInput(provider, out var absent) && absent is not null ? absent : null;
            errors = Array.Empty<string>();
            return true;
        }
    ...
    /// <summary>Converts a value from outside the model and creates the value object from it (throws ValueObjectValidationException on violation; an absent value returns null unless claimed through <see cref="IValueObject{TSelf}.TryConvertAbsentInput"/>).</summary>
    /// <param name="raw">The value read from the source.</param>
    /// <param name="provider">The culture the text is written in; <c>null</c> uses the invariant culture.</param>
