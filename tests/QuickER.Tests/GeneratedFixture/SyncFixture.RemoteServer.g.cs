...
// Content hash: sha256:f48556fbbf35eca359ba2a37b80cd02e47a275c2dec80c00859ee1f11649b2f0
#nullable enable
...
public static partial class GeneratedRemoteEndpoints
{
    ...
    private static void MapSyncOrderEndpoints(RouteGroupBuilder group, bool allowUnboundedUploads)
    {
        ...
        group.MapPost(
            "SyncOrder/GetHeadlines",
            (HttpContext context) =>
                RemoteServerEngine.ExecuteAsync(
                    context,
                    async () =>
                    {
                        var request = await RemoteServerEngine.ReadRequestAsync<SyncOrder_GetHeadlinesRequest>(context).ConfigureAwait(false);
                        var repository = RemoteServerEngine.Repository<ISyncOrderRemoteRepository>(context);
                        return (object?)await repository.GetHeadlinesAsync(request.MinOrderId, context.RequestAborted).ConfigureAwait(false);
                    }
                )
        );
    ...
    /// <summary>Request body for GetHeadlines (SyncOrder).</summary>
    private sealed record SyncOrder_GetHeadlinesRequest(int MinOrderId);
