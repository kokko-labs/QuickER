...
// Content hash: sha256:6780421dbe46509f7a5e1a15c0b6f47ada1d5b9ed9b7e994ee98a73f0eabaa61
#nullable enable
...
public static partial class GeneratedRemoteEndpoints
{
    ...
    private static void MapDocumentEndpoints(RouteGroupBuilder group, bool allowUnboundedUploads)
    {
        ...
        group.MapPost(
            ...
            (HttpContext context) =>
                RemoteServerEngine.ExecuteAsync(
                    ...
                    async () =>
                    {
                        var request = await RemoteServerEngine.ReadRequestAsync<Document_GetByTitleRequest>(context).ConfigureAwait(false);
                        var repository = RemoteServerEngine.Repository<IDocumentRemoteRepository>(context);
        ...
        RemoteServerEngine.MapBinaryColumn<int>(
            ...
            static (context, id, destination) =>
                RemoteServerEngine.Repository<IDocumentRemoteRepository>(context)
                    .ReadPayloadAsync(id, destination, context.RequestAborted),
            static (context, id, source, length) =>
                RemoteServerEngine.Repository<IDocumentRemoteRepository>(context)
                    .WritePayloadAsync(id, source, length, context.RequestAborted)
        );
        ...
        RemoteServerEngine.MapBinaryColumn<int>(
            ...
            static (context, id, destination) =>
                RemoteServerEngine.Repository<IDocumentRemoteRepository>(context)
                    .ReadThumbAsync(id, destination, context.RequestAborted),
            static (context, id, source, length) =>
                RemoteServerEngine.Repository<IDocumentRemoteRepository>(context)
                    .WriteThumbAsync(id, source, length, context.RequestAborted)
        );
    }

    /// <summary>Request body for GetByTitle (Document).</summary>
    private sealed record Document_GetByTitleRequest(string Title);
    ...
    {
        RemoteServerEngine.MapCrud<DocumentNoteEntity, int, IDocumentNoteRemoteRepository>(
            group,
            "DocumentNote"
        );
    }
