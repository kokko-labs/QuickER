...
/// 観点別の <c>[Fact]</c> は「除外が効くQuickER 版 Repository 版（<see cref="BinaryColumnAdoRuntimeTests"/>）」と
...
public abstract class BinaryColumnRuntimeTestsBase : IDisposable
{
    ...
    /// シードは各派生のリポジトリ経由（QuickER は全列 INSERT・EF Core は除外非適用で全列）で投入する。
    /// </remarks>
    ...
    // 「除外列を含めて取得する」効果はQuickER 版 Repository と EF Core で結果が同一（EF Core は元々全列）のため、
    // パリティを担保するべく本基底に [Fact] を置き、Ado / EF Core 両派生で同じテストを実行させる。
