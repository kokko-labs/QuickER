...
/// <summary>
/// B: <see cref="OracleSyncScriptBuilder"/> + <see cref="OracleSchemaSyncExecutor"/> による
/// 説明（COMMENT ON）の同期往復を検証する統合テスト。設定・更新・削除の 3 パターンを実 DB で確認する。
/// </summary>
[Trait("Category", "Integration")]
[Collection(OracleContainerCollection.Name)]
public sealed class OracleCommentSyncIntegrationTests(OracleContainerFixture fixture)
{
    ...
    /// </summary>
    /// <remarks>Oracle は <c>COMMENT ... IS ''</c>（空文字）が削除に相当する（<c>IS NULL</c> 構文は無い）</remarks>
    ...
    public async Task CommentSync_SetUpdateDelete_RoundTrips()
    {
        ...
        await fixture.ExecuteAsync(
            "CREATE TABLE \"items\" (\"id\" NUMBER(10) NOT NULL, \"name\" VARCHAR2(50) NULL, "
                + "CONSTRAINT \"PK_items\" PRIMARY KEY (\"id\"));",
            Ct
        );

        var settings = fixture.ToDbConnectionSettings();
        var builder = new OracleSyncScriptBuilder();
        var executor = new OracleSchemaSyncExecutor();
        var importer = new OracleSchemaImporter();
    ...
    private static async Task ApplyAsync(
        OracleSyncScriptBuilder builder,
        OracleSchemaSyncExecutor executor,
    ...
    private static async Task<Entity> ImportSingleAsync(
        OracleContainerFixture fixture,
        OracleSchemaImporter importer,
        string table
    )
    {
