...
/// 観点別の <c>[Fact]</c> は「除外が効くRepository (QuickER) 版（<see cref="BinaryColumnAdoRuntimeTests"/>）」と
...
public abstract class BinaryColumnRuntimeTestsBase : IDisposable
{
    ...
    /// <summary>文書リポジトリを生成する（QuickER = AddGeneratedRepositories / EF = AddGeneratedEfCoreRepositories）</summary>
    ...
    /// シードは各派生のリポジトリ経由（QuickER は全列 INSERT・EF は除外非適用で全列）で投入する。
    /// </remarks>
    ...
    // 「除外列を含めて取得する」効果はRepository (QuickER) と EF Core で結果が同一（EF は元々全列）のため、
