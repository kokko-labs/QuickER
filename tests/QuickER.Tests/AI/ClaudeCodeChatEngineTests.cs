...
public class ClaudeCodeChatEngineTests
{
    ...
    // 注: MockProjectAgentRunnerTests の同名フェイクとは挙動が本質的に異なるため共有化せず個別に残す。
    // こちらはストリーミング（ScriptedText・既定は無送出）・アウトカムのキュー再生・resume/probe の記録が主眼で、
    // あちらは固定テキスト送出・単一アウトカム・OnRun 副作用/例外・プロンプト捕捉が主眼（統合すると既定挙動が変わる）。
