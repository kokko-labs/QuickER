...
/// <summary>
/// <see cref="PgIdentifier"/> の識別子クォート・エスケープ境界を検証するテストクラス。
/// PostgreSQL は識別子を <c>"..."</c> で二重引用符クォートし、内部の <c>"</c> を <c>""</c> へ二重化してエスケープする。
/// </summary>
public class PgIdentifierTests
{
    ...
    public void Quote_ContainingDoubleQuote_EscapesBeforeWrapping()
    {
        ...
        PgIdentifier.Quote("User\"Name").Should().Be("\"User\"\"Name\"");
    }
    ...
    {
        PgIdentifier.Quote("pub\"lic.us\"ers").Should().Be("\"pub\"\"lic\".\"us\"\"ers\"");
    }
    ...
    public void Quote_WithWhitespaceSymbolsAndJapanese_PassesThroughUnescaped()
    {
        ...
        PgIdentifier.Quote(name).Should().Be("\"顧客 名前#1\"");
        PgIdentifier.QuoteSimple(name).Should().Be("\"顧客 名前#1\"");
        PgIdentifier.Escape(name).Should().Be(name);
    }
    ...
    {
        PgIdentifier.Quote(name!).Should().Be("\"\"");
    }
    ...
    {
        PgIdentifier.QuoteSimple(name!).Should().Be("\"\"");
    }
    ...
    {
        PgIdentifier.Escape(name!).Should().Be(string.Empty);
    }
}
