...
public sealed class SqlServerTranslatorOperatorRuntimeTests(SqlServerContainerFixture fixture)
{
    ...
    /// </summary>
    [Fact(
        DisplayName = "[SqlServer演算子] NotEqual（<>）が等値行を除外し、NULL 列は補償により一致（含める）"
    )]
    ...
    /// <summary>
    /// IN / NOT IN の NULL 意味論が C#（インメモリ実行器の式木コンパイル）・EF Core と一致することを実 SQL Server で確認する。
    /// </summary>
    /// <remarks>
    ...
    /// 後者は列側を無条件補償することで揃う。SQLite 版 <see cref="SqliteTranslatorOperatorRuntimeTests"/> と
    /// 同じ観測結果になる（方言差は識別子のクォートだけ）。
    /// </remarks>
    [Fact(DisplayName = "[SqlServer演算子] IN / NOT IN の NULL 意味論が C# と一致する")]
    ...
    /// </summary>
    [Fact(
        DisplayName = "[SqlServer演算子] 全要素が null のリストの IN / NOT IN が NULL 行だけを二分する"
    )]
    ...
    /// </summary>
    [Fact(DisplayName = "[SqlServer演算子] 空コレクションの NOT IN が 1=1（常真）で全行を返す")]
