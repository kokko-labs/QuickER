...
crow's foot 記法を使用して、テーブル、列、主キー、外部キー、関連を視覚的に設計できます。
...
QuickER が生成したコードは、インラインで生成されるランタイム部分を含めて利用者の成果物です。生成コードは、QuickER 本体のライセンスによる制限を受けず、自由に利用、改変、配布できます。

QuickER 本体は、プロジェクトごとに次のライセンスを適用しています。

| 対象                                       | ライセンス                                               |
| ---------------------------------------- | --------------------------------------------------- |
| ER デザイナ、入出力、DDL 生成、DB 取込・同期、ランタイムパッケージなど | [MIT License](LICENSE)                              |
| AI 機能とコード生成関連プロジェクト                      | [PolyForm Noncommercial 1.0.0](LICENSE-NC.md) ＋追加許諾 |

現在は、AI 機能とコード生成機能を商用利用を含めて無料で利用できます。

将来、AI 機能と一部の DB アクセスコード生成機能について、商用利用のみ有償化する可能性があります。その場合も、個人・非商用利用と、Entity / EditModel / Mapper の基本生成は無料で提供する方針です。有償化する場合は事前に告知し、既存利用者には移行期間を設けます。

これらの約束は [LICENSE-NC.md](LICENSE-NC.md) の「Additional Grants（追加許諾）」節として条文化されており、現在の商用利用はライセンスファイル自体の許諾に基づきます。

正式な条件については、必ず各ライセンスファイルを参照してください。
