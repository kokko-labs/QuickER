"""ScribanCSharpRenderer.cs を新しい内容で上書きします。"""
...
internal sealed class ScribanCSharpRenderer
{
	private const string TemplateText = \"\"\"
		...
		{
			public {{ mapper.ui_model_class_name }} CommitToUiModel({{ mapper.entity_class_name }} entity)
			{
				var uiModel = new {{ mapper.ui_model_class_name }}();
				LoadFrom(entity, uiModel);
				return uiModel;
			}

			public void CommitToEntity({{ mapper.ui_model_class_name }} uiModel, {{ mapper.entity_class_name }} entity)
			{
		{{ for prop in mapper.scalar_properties }}        entity.{{ prop.property_name }} = uiModel.{{ prop.property_name }};
		{{ end }}    }

			private static void LoadFrom({{ mapper.entity_class_name }} entity, {{ mapper.ui_model_class_name }} uiModel)
			{
				uiModel.RevertInput();
		{{ for prop in mapper.scalar_properties }}        uiModel.{{ prop.binding_property_name }} = entity.{{ prop.property_name }}?.ToString() ?? string.Empty;
		{{ end }}    }
		}
		{{~ end ~}}
		\"\"\";
	...
	public string Render(CSharpGenerationModel model, CodeGenerationOptions options)
	{
		...
		var scriptObject = new Scriban.Runtime.ScriptObject
		{
			...
			["ui_model_classes"] = model.UiModelClasses,
