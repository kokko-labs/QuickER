...
- **Value objects can be created from a raw value** — `TryCreateFrom` and `CreateFrom` build a value object from a value that is not already the underlying type (`object?`), such as a CSV field or a spreadsheet cell. The reading culture is passed as an `IFormatProvider`, and numeric text may carry group separators. A blank cell (`null`, `DBNull`, or an empty string) is not a violation: the call succeeds with a null result. A new self-typed interface `IValueObject<TSelf>` leaves the underlying type out of the type parameters, so import code is written once as `where T : IValueObject<T>`. A type can also accept an input shape of its own - a name for an enumeration-like type, say - through the `TryCreateFromCustom` hook (generated value objects expose it as the `CreateFromCustom` partial); `TryCreateFrom` consults the hook first on every call form, the concrete-type-name spelling included (see "Creating from a raw value" in docs/code-generation.md)
...
- **Breaking**: `IValueObject<TSelf, TValue>` now inherits the new self-typed `IValueObject<TSelf>` and gained `TryCreateFrom` / `CreateFrom` (plus `TryGetDefined` and `TryCreateFromCustom`, which have default implementations). A type deriving from `ValueObjectBase` is unaffected, since the base class supplies them; a type implementing the interface by hand without that base now has to implement the two factories.
