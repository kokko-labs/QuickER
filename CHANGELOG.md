...
- **Value objects can be created from a raw value** — `TryCreateFrom` and `CreateFrom` build a value object from a value that is not already the underlying type (`object?`), such as a CSV field or a spreadsheet cell. The reading culture is passed as an `IFormatProvider`, and numeric text may carry group separators. A blank cell (`null`, `DBNull`, or an empty string) is not a violation: the call succeeds with a null result. A new self-typed interface `IValueObject<TSelf>` leaves the underlying type out of the type parameters, so import code is written once as `where T : IValueObject<T>` (see "Creating from a raw value" in docs/code-generation.md)
- **Value objects that accept only declared instances** — implementing `TryGetDefined` makes `Create` and `TryCreate` return the instances declared as `static readonly` rather than building new ones, which is how a closed set - a mode, a category, a status - becomes a value object. The hook sits one step outside `New`, on `Create` / `TryCreate`, so a row read from the database and a value restored from JSON return the declared instance too, and **a generated value object becomes an enumeration from a partial of your own** - no generator option involved (see "Accepting only declared instances" in docs/code-generation.md)
...
- **Breaking**: `IValueObject<TSelf, TValue>` now inherits the new self-typed `IValueObject<TSelf>` and gained `TryCreateFrom` / `CreateFrom` (plus `TryGetDefined`, which has a default implementation). A type deriving from `ValueObjectBase` is unaffected, since the base class supplies them; a type implementing the interface by hand without that base now has to implement the two factories.
