...
- **An enumeration-like value object is now declared with one attribute — `[DeclaredInstance]`** — mark the `static readonly` instances of a generated (or hand-written) value object and the whole enumeration behavior comes together: every creation path — database reads and JSON restore included — returns the declared instances, any other value is rejected as a validation error (wording replaceable through `ValueObjectValidationMessages.ValueNotDeclared`), `GetDeclaredInstances()` lists them in declaration order, and a `[Display(Name = ...)]` on the field is available as `DeclaredDisplayName` for a one-line `DisplayValue` override. A declared instance can also claim its input notation for `TryCreateFrom` / `CreateFrom`: `InputText = "○"` reads that exact string as the instance, and `ClaimsAbsent = true` reads a blank cell (null / DBNull / an empty string) as it — a "mark or blank" flag column closes in two declarations, replacing the `ConvertCustomInput` / `ConvertAbsentInput` boilerplate for the fixed-notation case (the hooks remain and win when both are present). The hand-written table, `GetDefinedInstance` and the membership `OnValidate` are no longer needed for the common case; the hooks remain available and compose with the attribute (the hook wins the lookup, a custom `OnValidate` runs in addition). Initialize the fields with `new(...)`, not `Create`. See the value-object section of [docs/code-generation.md](docs/code-generation.md)
