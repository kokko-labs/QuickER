...
- **Value objects can be created from a raw value** — `TryCreateFrom` and `CreateFrom` build a value object from a value that is not already the underlying type (`object?`), such as a CSV field or a spreadsheet cell. The reading culture is passed as an `IFormatProvider`, and numeric text may carry group separators. A blank cell (`null`, `DBNull`, or an empty string) is not a violation: the call succeeds with a null result. A new self-typed interface `IValueObject<TSelf>` leaves the underlying type out of the type parameters, so import code is written once as `where T : IValueObject<T>`. A type can also accept an input shape of its own - a name for an enumeration-like type, say - through the `TryConvertCustomInput` hook (generated value objects expose it as the `ConvertCustomInput` partial); `TryCreateFrom` consults the hook first on every call form, the concrete-type-name spelling included (see "Creating from a raw value" in docs/code-generation.md)
- **Value objects that accept only declared instances** — implementing `TryGetDefined` makes `Create` and `TryCreate` return the instances declared as `static readonly` rather than building new ones, which is how a closed set - a mode, a category, a status - becomes a value object. The hook sits one step outside `New`, on `Create` / `TryCreate`, so a row read from the database and a value restored from JSON return the declared instance too, and **a generated value object becomes an enumeration by implementing the `GetDefinedInstance` partial hook** - no generator option involved (see "Accepting only declared instances" in docs/code-generation.md)
...
- **Breaking**: `IValueObject<TSelf, TValue>` now inherits the new self-typed `IValueObject<TSelf>` and gained `TryCreateFrom` / `CreateFrom` (plus `TryGetDefined` and `TryConvertCustomInput`, which have default implementations). A type deriving from `ValueObjectBase` is unaffected, since the base class supplies them; a type implementing the interface by hand without that base now has to implement the two factories.
- **Breaking**: a value object's `Validate(TValue value, ICollection<string> errors)` now returns `bool` instead of `void` (`true` means *that call* found no violation). When the collection is shared across several values, its count cannot tell you whether a particular value passed, so the verdict is now returned. Violations are collected exactly as before, and a call site that ignores the result still compiles unchanged.
- **Breaking**: a value object's `OnValidate` partial hook now receives the error list by reference and possibly unallocated (`static partial void OnValidate(T value, ref List<string>? errors)`); allocate it only when adding the first violation (`(errors ??= new List<string>()).Add(...)`), the same shape `ValidateCore` itself uses. Previously the generated `ValidateCore` allocated a fresh `List<string>` on every single creation of any type that implements the hook - even when the value passed, per column per row on reads. An existing implementation stops compiling (CS0759) until its signature is updated; behavior is otherwise identical.
...
- **Generated code got substantially smaller with identical behavior**: the per-entity boilerplate of the sync journaling decorator, the direct sync source, the mapper's create methods, and the edit model's sibling-navigation helpers moved into generic base classes (`JournalingRepositoryBase` / `DirectSyncSourceBase` and their versioned/versionless subclasses in `QuickER.Runtime.Sync`; `CreateEntityCore` / `CreateEditModelCore` on `MapperBase`; a new self-typed `EditModelBase<TSelf>` layer in `QuickER.Runtime`; and the machinery of the edit model's two per-column setters — normalization, display reset, parsing, value-object conversion — as `AcceptBindingInput` / `AfterConfirmedValueSet` and the conversion helpers on `EditModelBase`; and the uniqueness pre-check's matching loop as `UniquenessChecker`, with each entity's constraints emitted once as a dialect-neutral data table beside the contract instead of being repeated per implementation target). Generated classes now carry only their own identity (table name, key handling, SQL text, assignments, messages, hooks). `IValueObject<TSelf, TValue>` also gained a `DisplayName` member with a default implementation (non-breaking; the generated static property implements it). If you hand-derive from `MapperBase`, note it now requires `new()`-constructible type arguments and an `ApplyToEditModel` implementation.
