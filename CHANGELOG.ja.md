...
- 初回公開
  ...
  - C# コード生成（Entity / EditModel / Mapper＋DB アクセス 3 択: なし / QuickER 版 Repository / EF Core。同一インターフェイスで DI 登録 1 行の差し替え・ランタイム NuGet パッケージ参照モード）
  ...
  - ライセンス構成: コア＝MIT、AI 機能群・コード生成系・MCP ツール実行ホスト（8 プロジェクト）＝PolyForm Noncommercial 1.0.0＋追加許諾（現在は商用利用を含め全員無料。基本コード生成の商用利用は恒久許諾。LICENSE-NC.md の「Additional Grants」節として条文化。平易な解説は LICENSING.ja.md）
